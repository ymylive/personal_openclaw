...
const CLIENT_ID_ENV = "OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_ID";
const CLIENT_SECRET_ENV = "OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_SECRET";
...
function getGoogleOAuthClient(): { clientId: string; clientSecret: string } {
  const clientId = process.env[CLIENT_ID_ENV]?.trim();
  const clientSecret = process.env[CLIENT_SECRET_ENV]?.trim();

  if (!clientId || !clientSecret) {
    throw new Error(
      `Missing Google Antigravity OAuth credentials. Set ${CLIENT_ID_ENV} and ${CLIENT_SECRET_ENV}.`,
    );
  }
