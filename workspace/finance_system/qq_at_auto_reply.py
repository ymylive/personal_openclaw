...
TEXT_FILE_EXTS = {'.txt', '.md', '.markdown', '.json', '.csv', '.py', '.c', '.cpp', '.cc', '.h', '.hpp', '.java', '.js', '.ts', '.html', '.xml', '.yml', '.yaml', '.log'}
...
def load_blacklist() -> set[int]:
    ...
    try:
        ...
        return {int(item['group_id']) for item in groups if str(item.get('group_id', '')).isdigit()}
    except Exception:
        return set()
def state_path_for(group_id: int) -> Path:
    STATE_DIR.mkdir(parents=True, exist_ok=True)
    return STATE_DIR / f'group_{group_id}.json'
def load_state(group_id: int):
    ...
    for path in paths:
        ...
        try:
            ...
            if not isinstance(replied_ids, list):
                replied_ids = []
            state['replied_message_ids'] = [int(x) for x in replied_ids if str(x).isdigit()][-MAX_REPLIED_IDS:]
            return state
        except Exception:
            pass
    return {'last_message_id': 0, 'replied_message_ids': []}
def save_state(group_id: int, state):
    state = dict(state)
    replied_ids = state.get('replied_message_ids') or []
    state['replied_message_ids'] = replied_ids[-MAX_REPLIED_IDS:]
    state_path_for(group_id).write_text(json.dumps(state, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
async def call(ws, action, params, echo):
...
def update_state(group_id: int, state, replied_id_list, max_seen_id):
...
def extract_image_urls_from_raw(raw: str) -> List[str]:
    return [m.group(1).strip() for m in IMAGE_RE.finditer(raw or '') if m.group(1).strip()]
def looks_like_media_request(text: str) -> bool:
    t = clean_text(text).lower()
    return bool(IMAGE_INTENT_RE.search(t) or FILE_INTENT_RE.search(t) or '这个' in t or '这个题' in t or '最新' in t)
...
def display_name(message: Dict[str, Any]) -> str:
    ...
    return str(sender.get('nickname') or sender.get('card') or message.get('user_id') or 'unknown')


def extract_image_urls_from_segments(message: Any) -> List[str]:
    urls = []
    for seg in extract_segments(message):
